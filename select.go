...
func (s SelectQuery) ToMaps() ([]map[string]any, error) {
	rows, err := s.Build().WithContext(s.ctx).Rows()
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var resultTmp = make(map[string]any, count)
		for i, column := range columns {
			if b, ok := (*(values[i].(*interface{}))).([]byte); ok {
				resultTmp[column] = string(b)
			} else {
				resultTmp[column] = values[i]
			}
		}
		result = append(result, resultTmp)
	}
	if result == nil && rows.Err() == nil {
...
func (s SelectQuery) ToMap() (map[string]any, error) {
	rows, err := s.Build().WithContext(s.ctx).Rows()
	if err != nil {
		return nil, err
	}
